// VulkanNX.cpp : Defines the entry point to the NintendoSDK application.
//
...
private:
	...
	const std::string VERTEX_PATH = "./shaders/vert.spv";
	...
	const std::string MODEL_PATH = "rom:/models/chalet.obj";
	const std::string TEXTURE_PATH = "rom:/textures/chalet.jpg";
	const std::string VERTEX_PATH = "rom:/shaders/vert.spv";
	const std::string FRAG_PATH = "rom:/shaders/frag.spv";
	const std::vector<const char*> validationLayers = {
	...
	std::vector<VkSemaphore>	renderFinishedShemaphores;
	...
	void createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory)
	{
		...
		VkMemoryRequirements memRequeriments;
		vkGetImageMemoryRequirements(device, image, &memRequeriments);

		VkMemoryAllocateInfo allocInfo = {};
		allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
		allocInfo.allocationSize = memRequeriments.size;
		allocInfo.memoryTypeIndex = findMemoryType(memRequeriments.memoryTypeBits, properties);
	...
	void createSyncObjects()
	{
		...
		renderFinishedShemaphores.resize(MAX_FRAMES_IN_FLIGHT);
		...
		for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
			if (vkCreateSemaphore(device, &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) != VK_SUCCESS ||
				vkCreateSemaphore(device, &semaphoreInfo, nullptr, &renderFinishedShemaphores[i]) != VK_SUCCESS ||
	...
	void createSurface()
	{
		...
		result = vkCreateViSurfaceNN(instance, &createInfo, NULL, &surface);
...
#elif defined(__NX__)
			VkExtent2D actualExtend = {
				static_cast<uint32_t>(1280),
				static_cast<uint32_t>(720)
			};
#endif
			return actualExtend;
		}
	...
	void drawFrame()
	{
		...
		VkSemaphore signalSemaphores[] = { renderFinishedShemaphores[currentFrame] };
	...
	void cleanup()
	{
		...
		for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
			vkDestroySemaphore(device, renderFinishedShemaphores[i], nullptr);
	...
	static std::vector<char> readFile(const std::string& filename)
	{
		...
		openResult = nn::fs::OpenFile(&handle, filename.c_str(), nn::fs::OpenMode_Read);
